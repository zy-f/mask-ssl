...
def load_model(arch, weight_name, n_cls=100, mode='full'):
...
def main_ftune(cfg):
    ...
    for (layer, patch_size, mask_ratio) in SWEEP_FTUNE:
        ...
        cfg.weight_name = f"layer{layer}_patch{patch_size}_ratio{int(mask_ratio*100)}-best"
        model = load_model(cfg.arch, cfg.weight_name, n_cls=cfg.n_cls, mode='ftune')
